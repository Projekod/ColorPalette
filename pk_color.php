...
class pk_color {
    ...
    public function getIt($picture=""){
        ...
        if(!empty($picture)){$this->picture = $picture;}

        if(!file_exists($this->picture)){
            throw new Exception("Resim dosyası bulunamadı.");
        }
        ...
        if($_sizes === false){
            throw new Exception("Resim boyuları alınamadı.");
        }

        $_img = @imagecreatefromstring(file_get_contents($this->picture));

        if(!$_img){
            throw new Exception("Resim dosyası hatalı.");
        }
